...
export default function UserLogin() {
  ...
  useEffect(() => {
    if (state?.error?.startsWith("Banned:")) {
      setBanReason(state.error.replace("Banned: ", ""));
  ...
  return (
    <div style={{ background: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ padding: "0 24px 40px", maxWidth: 480, margin: "0 auto" }}>
        {state?.error && !state.error.startsWith("Banned:") && (
          <div className="auth-error"><AlertCircle size={18} /> {state.error}</div>
        )}
